...
def set_account_enabled(headers: dict, user_id: str, enabled: bool) -> int:
    ...
    # 204 = sucesso; 200 também pode ocorrer em alguns proxies, mas 204 é o esperado.
...
def main(mytimer: func.TimerRequest):
    try:
        ...
        for user in iter_group_members(headers, group_id):
            ...
            status = set_account_enabled(headers, uid, enabled=False)
            logging.info(f"[disable] {upn} ({uid}) -> status={status}")
            count += 1

        logging.info(f"Concluído. {count} usuário(s) processado(s) para bloqueio.")

    except Exception as e:
        logging.exception(f"ERRO na disable_users: {e}")
        # Não relança para que o host registre o log; se preferir, raise para marcar falha dura.
